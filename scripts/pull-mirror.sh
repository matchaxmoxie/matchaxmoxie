...
rsync -a --delete --exclude='.DS_Store' \
  --exclude='pull-mirror.sh' \
  --exclude='pull-mirror-if-changed.sh' \
  --exclude='launchd.matchaxmoxie-sync.plist.example' \
  --exclude='publish-site.sh' \
  "$TMP/scripts/" "matchaxmoxie/scripts/"
...
chmod +x "matchaxmoxie/scripts/pull-mirror.sh" "matchaxmoxie/scripts/publish-site.sh"

echo "Done. Synced $REMOTE/$BRANCH into matchaxmoxie/ (site, latex, scripts, docs, root README)."
