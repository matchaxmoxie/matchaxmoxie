...
This file is **not** copied from the mirror. `pull-mirror.sh` overwrites `README.md` and `docs/` from the mirror, so long-lived SU26-only notes live here.
...
1. Copy [`launchd.matchaxmoxie-sync.plist.example`](launchd.matchaxmoxie-sync.plist.example) to `~/Library/LaunchAgents/` and rename (for example `com.yourname.su26-matchaxmoxie-sync.plist`).
2. Replace `SU26_ROOT_PLACEHOLDER` with your absolute path to the SU26 repo (keep the path inside single quotes in the `cd` string).
3. Load once: `launchctl load ~/Library/LaunchAgents/com.yourname.su26-matchaxmoxie-sync.plist`
