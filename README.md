# Jade Zhao (Matcha & Moxie)

### Brand & Operations Lead · GTM Coordinator · Creative Entrepreneur

📍 Bloomington, IN · May 2027 · open to relocation

📧 [Email](mailto:jlzhao@iu.edu) · 🌐 [Live site](https://matchaxmoxie.github.io/matchaxmoxie/) · 💼 [The briefcase](https://jadexzhao.github.io/jadexzhao/) · 📸 [@zhao.langxi](https://instagram.com/zhao.langxi) · 🔗 [LinkedIn](https://www.linkedin.com/in/jadexzhao/) · [@matchaxmoxie](https://github.com/matchaxmoxie) · [@jadexzhao](https://github.com/jadexzhao)

> **Restaurant kid turned brand & ops lead.** I grew up where the only metric that mattered was whether the food hit the table hot. That wiring stuck. I coordinate programs, clear paths for the next person, and keep creative + GTM lanes loud on purpose but anchored by the work.

---

## The Co-Founder Setup

One human, two GitHub accounts. **This account is brand & ops.** Code ships on the other.

| | **[@matchaxmoxie](https://github.com/matchaxmoxie)** · brand & ops *(you are here)* | **[@jadexzhao](https://github.com/jadexzhao)** · ships |
| --- | --- | --- |
| **Role** | FASE MAP coordinator, UTA, consulting, campus creator | GTM eng, ServeIT web lead, code, portfolio |
| **Site** | [matchaxmoxie.github.io](https://matchaxmoxie.github.io/matchaxmoxie/) | [the briefcase](https://jadexzhao.github.io/jadexzhao/) |

Stack talk, featured ships, and engineering detail live on [`@jadexzhao`](https://github.com/jadexzhao). Here is where the program runs.

[![Live site](https://img.shields.io/badge/Live_site-matchaxmoxie-F2C4CE?style=for-the-badge&logo=html5&logoColor=3D2B2B)](https://matchaxmoxie.github.io/matchaxmoxie/)
[![The briefcase](https://img.shields.io/badge/The_briefcase-jadexzhao.github.io-34472C?style=for-the-badge&logo=githubpages&logoColor=white)](https://jadexzhao.github.io/jadexzhao/)
[![GitHub @matchaxmoxie](https://img.shields.io/badge/GitHub-@matchaxmoxie-24292f?style=for-the-badge&logo=github&logoColor=white)](https://github.com/matchaxmoxie)
[![GitHub @jadexzhao](https://img.shields.io/badge/GitHub-@jadexzhao-24292f?style=for-the-badge&logo=github&logoColor=white)](https://github.com/jadexzhao)
[![Instagram @zhao.langxi](https://img.shields.io/badge/Instagram-@zhao.langxi-00a86b?style=for-the-badge&logo=instagram&logoColor=white)](https://instagram.com/zhao.langxi)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-jadexzhao-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/jadexzhao/)
[![Email](https://img.shields.io/badge/Email-jlzhao%40iu.edu-5F7D4F?style=for-the-badge&logo=gmail&logoColor=white)](mailto:jlzhao@iu.edu)

---

## Currently Running

* **FASE MAP Coordinator & UTA** | *[FASE MAP](https://matchaxmoxie.github.io/matchaxmoxie/fase-map.html) · IU Luddy*  
  Mentor apprenticeship program, Canvas publish checklists, study resources, and handoffs for 50+ students. Clearing the path for whoever comes next.

* **ServeIT Website Team** | *[IU ServeIT](https://serveit.luddy.indiana.edu/serve-ai/index.html) (nonprofit tech clinic)*  
  Ops & handoff framing: scoped builds community orgs can maintain without a developer on call. Engineering lead and stack live on [`@jadexzhao`](https://github.com/jadexzhao) / [the briefcase](https://jadexzhao.github.io/jadexzhao/).

* **Campus Creator Lane** | *[@zhao.langxi](https://instagram.com/zhao.langxi)*  
  Plants, matcha, lowercase truth. Loud on purpose, always anchored by the work.

* **Potstickers Ops** | *[Ordering](https://potstickers24.cloveronline.com/) · [pstickers24](https://github.com/jadexzhao/pstickers24)*  
  Family restaurant site + Clover online ordering. First production ship, still the reason the dual-account setup exists. Site code on `@jadexzhao`; brand & seasonal ops pulse here.

*Light GTM context:* also in the mix at a stealth YC startup (NYC). Demo/PoC engineering detail sits on the code account, not this one.

---

## Global & Academic

* **Informatics @ IU Luddy** · graduating May 2027 · first-gen · Hudson & Holland Scholar
* **Spring 2026** · Complutense Madrid
* **Languages** · English, Mandarin, Fuzhounese, Spanish (improving)
* **Roots** · 福州 · Fujian ↔ Midwest wiring
* **Open to** · Brand ops · GTM coordination · Solutions / program ops · open to relocation

---

## Active Repos

| Repo | Why it lives here / nearby |
|------|----------------------------|
| [`matchaxmoxie/matchaxmoxie`](https://github.com/matchaxmoxie/matchaxmoxie) | This repo. Consulting surface + [FASE MAP](https://matchaxmoxie.github.io/matchaxmoxie/fase-map.html) coordinator tooling |
| [`jadexzhao/jadexzhao`](https://github.com/jadexzhao/jadexzhao) | Shipped portfolio: ServeIT builds, duck farm, [the briefcase](https://jadexzhao.github.io/jadexzhao/) |
| [`jadexzhao/pstickers24`](https://github.com/jadexzhao/pstickers24) | Family restaurant site; orders at [potstickers24.cloveronline.com](https://potstickers24.cloveronline.com/) |

---

## Learning in Public

Coming soon (notes, not products yet):

* WeChat mini-program notes
* China AI / cybersec watchlist
* Fujian ↔ Midwest dev diary

---

*The duck never got a seat at the zodiac table, so we made one.* 🦆 **鸭年 2026** · [catch one on the virtual duck farm](https://jadexzhao.github.io/jadexzhao/duck-farm/).
