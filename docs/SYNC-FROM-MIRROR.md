...
| Script | Role |
| :--- | :--- |
| [`scripts/pull-mirror.sh`](../scripts/pull-mirror.sh) | Full archive sync from remote `matchaxmoxie` `main` into this folder. |
| [`scripts/pull-mirror-if-changed.sh`](../scripts/pull-mirror-if-changed.sh) | `git fetch`, then run `pull-mirror.sh` only if `main` advanced. Writes `.git/matchaxmoxie-last-pulled`. |
...
1. Copy [`scripts/launchd.matchaxmoxie-sync.plist.example`](../scripts/launchd.matchaxmoxie-sync.plist.example) to `~/Library/LaunchAgents/` and rename (for example `com.yourname.ucm-matchaxmoxie-sync.plist`).
